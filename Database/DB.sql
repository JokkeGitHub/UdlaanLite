...
CREATE TABLE OneTimeLoan(
	...
	qr_id varchar(5) references pc(QR_ID),
	date_of_loan date
);
...
language plpgsql as $$
begin
	INSERT INTO OneTimeLoan VALUES (tlf_number, qr_id, todays_date);
	UPDATE pc SET in_stock = false WHERE QR_ID = qr_id;
	commit;
end;$$;
...
Language plpgsql as $$
begin
	Delete from OneTimeLoan Where QR_id = qr_id;
	Update pc set in_stock = true Where QR_ID = qr_id;
	commit;
end;$$;
...
language plpgsql as $$
begin
	insert into Udlaant Values (laanerID, qr_id, start_date, end_date);
	Update pc set in_stock = false Where QR_ID = qr_id;
	commit;
end;$$;
...
language plpgsql as $$
begin
	Delete From laaner where QR_ID = qr_id;
	Update pc set in_stock = true Where QR_ID = qr_id;
	commit;
end;$$;